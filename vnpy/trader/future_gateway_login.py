...
from vnpy.trader.tqz_extern.tqz_constant import TQZFutureGatewaySettingKey
...
class TQZGatewayLogin:
    ...
    def tqz_login_accounts(self, account_app_classes: [Type[BaseApp]]):
        ...
        ACCOUNT_NAME = TQZFutureGatewaySettingKey.ACCOUNT_NAME.value

        accounts_child_process: {str: None} = {}  # init dictionary of accounts child processes.
        for gateway_setting in gateway_settings:
            accounts_child_process[gateway_setting[ACCOUNT_NAME]] = None

        while True:  # 整个程序的主进程, 用一个死循环来维持生命周期, 每个账户每10s扫描一次
            trading = self.__check_trading_period()  # 判断当前是否是交易时间
            ...
            for gateway_setting in gateway_settings:
                ...
                # exit child process when not trading time.
                if not trading and accounts_child_process[gateway_setting[ACCOUNT_NAME]] is not None:
                    if not accounts_child_process[gateway_setting[ACCOUNT_NAME]].is_alive():
                        accounts_child_process[gateway_setting[ACCOUNT_NAME]] = None
                        print(f'子进程({gateway_setting[ACCOUNT_NAME]}) 关闭成功')
    ...
    def _run_account(self, gateway_setting, account_app_classes):
        ...
        for account_app_class in account_app_classes:  # account add classes.
            ...
            main_engine.write_log(f'({gateway_setting[ACCOUNT_NAME]}) 连接 {self.gateway_name} 接口')
        ...
        while True:
            ...
            if not trading:
                print(f'关闭子进程({gateway_setting[ACCOUNT_NAME]})')
                main_engine.close()
                sys.exit(0)
    ...
    def __tqz_update_accounts_data_jsonfile(cls):
        ...
        for gateway_settings in all_gateway_settings_content_list:
            for gateway_setting in gateway_settings:
                current_source_accounts_names.append(gateway_setting[TQZFutureGatewaySettingKey.ACCOUNT_NAME.value])
...
if __name__ == "__main__":
    # TQZGatewayLogin(gateway=CtpGateway, init_strategies_seconds=20).tqz_login_accounts(account_app_classes=[TQZDataDumpApp])
    pass
